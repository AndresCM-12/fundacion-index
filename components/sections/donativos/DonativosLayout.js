...
const navData = [
  {
    text: "Requisitos",
    link: "/donativos/requisitos",
  },
  {
    text: "Procedimiento",
    link: "/donativos/procedimiento",
  },
  {
    text: "Documentación",
    link: "/donativos/documentacion",
  },
  {
    text: "Criterios de no elegibilidad",
    link: "/donativos/criteriosnoelegibilidad",
  },
];

const DonativosLayout = ({ children, navData }) => {
  const [mobileOpen, setMobileOpen] = useState(false);
  return (
    <>
      {/* //MOBILE NAVBAR */}
      <div className="mobilenavbar lg:hidden bg-white flex items-center justify-between px-7 py-6">
        <p className="uppercase font-bold font-albra text-sm">Donativos</p>
        <div
              ...
              {navData.map((item, index) => (
                <div className="navbaritem my-3" key={index}>
                  <Button
                    ...
                    localUrl={item.link}
                    appearance="white"
                    className="text-black text-[15px] font-semibold hover:text-index-purple "
                  />
                </div>
              ))}
            </div>
...
export default DonativosLayout;
