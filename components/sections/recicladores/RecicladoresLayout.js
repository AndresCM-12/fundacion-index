...
const navData = [
  {
    text: "Empresas recicladoras",
    link: "/recicladores/empresas",
  },
  {
    text: "Requisitos",
    link: "/recicladores/empresas#requisitos",
  },
  {
    text: "Documentación",
    link: "/recicladores/documentacion",
  },
];

const RecicladoresLayout = ({ children, navData }) => {
  const [mobileOpen, setMobileOpen] = useState(false);
  return (
    <>
      {/* //MOBILE NAVBAR */}
      <div className="mobilenavbar lg:hidden bg-white flex items-center justify-between px-7 py-6">
        <p className="uppercase font-bold font-albra text-sm">Recicladores</p>
        <div
              ...
              {navData.map((item, index) => (
                <div className="navbaritem my-3" key={index}>
                  <Button
                    ...
                    localUrl={item.link}
                    appearance="white"
                    className="text-black text-[15px] font-semibold hover:text-index-yellow "
                  />
                </div>
              ))}
            </div>
...
export default RecicladoresLayout;
