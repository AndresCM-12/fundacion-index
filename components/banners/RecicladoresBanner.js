...
const RecicladoresBanner = ({ ...rest }) => {
  const [title, setTitle] = useState("");
  const [description, setDescription] = useState("");

  useEffect(() => {
    fetchTitleAndDescription().then((items) => {
      const section = items[3];
  ...
  return (
    <div
      className={`supercontainer min-h-screen h-screen w-screen bg-index-yellow ${rest.className}`}
      {...rest}
    >
      {/* MOBILE text */}
      ...
      <div className="imagecontainermobile lg:hidden w-screen">
        <img
          src="/images/recicladores/bannermobile.svg"
          alt="backgroundsvg"
      ...
      <div className="wrapper relative h-[450px] w-full hidden lg:flex justify-center  items-center">
        ...
        <div className="max-w-7xl flex items-center justify-around w-full ">
          ...
          {/* DEKSTOP SVG */}
          <div className="imagecontainer h-full w-1/2 z-10 ">
            <img
              src="/images/recicladores/bannerdesktop.svg"
              alt="backgroundsvg"
              className="w-full h-full"
            />
          </div>
...
export default RecicladoresBanner;
